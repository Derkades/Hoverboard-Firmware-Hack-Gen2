...
void ADC_init(void)
{
  ...
  // Configure ADC clock (APB2 clock is DIV1 -> 72MHz, ADC clock is DIV6 -> 12MHz)
	...
	// "SWRCST" sounds like "software", and "EXTTRIG_INSERTED_NONE" means "software trigger" in the new SDK version
...
void USART_HUGS_init(void)
{
	...
	dma_init(DMA_CH4, &dma_init_struct_usart); // MODIFIED struct pointer
...
void USART_Steer_COM_init(void)
{
	...
	dma_init_struct_usart.periph_width = DMA_PERIPHERAL_WIDTH_8BIT;
	dma_init_struct_usart.priority = DMA_PRIORITY_ULTRA_HIGH;
	dma_init(DMA_CH2, &dma_init_struct_usart); // MODIFIED struct pointer
